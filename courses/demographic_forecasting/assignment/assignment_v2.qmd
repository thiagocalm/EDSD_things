...
Male mortality from 1950 onwards for those aged 0-100 (i.e. exclude the 101-110+ age groups)
...
# Create array for simulation paths: [sim, age, year]
sim_paths <- array(NA, dim = c(n_sim, length(age), h))

# 2. SIMULATE FUTURE LOG-MORTALITY FOR EACH AGE --------------------

for (i in seq_along(age)) {
  y <- mort$yt[mort$Age == age[i]]
  fit <- auto.arima(y)
  sim_result <- simulate(fit, nsim = h, future = TRUE, bootstrap = TRUE, npaths = n_sim)
  sim_paths[, i, ] <- t(sim_result)
}

# 3. COMPUTE LIFE EXPECTANCY FOR EACH SIMULATION --------------------

# Function from Exercise 5 (already provided)
# e0.mx(age, mx, sex = "M")
...
# 5. DERIVE MEDIAN & 95% BOUNDS OF SIMULATIONS ----------------------
