...
hmd <- read_rds("courses/apc/data_input/hmd_dts_pop.rds")
head(hmd)
unique(hmd$code)
...
asfr <-
  read_rds("courses/apc/data_input/hfd_asfr.rds")
asfr
unique(asfr$code)
...
df_ratio <- asfr |>
  ...
  mutate(
    asfr_ratio = BEL/EST,
    log_ratio = log(asfr_ratio)
  )

# setting parameters

amin <- min(df_ratio$age)
amax <- max(df_ratio$age)
pmin <- min(df_ratio$year)
pmax <- max(df_ratio$year)

# plotting it...

# doing it for ratio without log we have some weird values in old and young ages...
# it is due to small values... let's see if taking the log it will work

df_ratio %>%
  ggplot() +
  aes(x = year, y = age, z = asfr_ratio) +
  geom_tile(aes(fill = asfr_ratio)) +
  # adding axis titles
  scale_fill_viridis(option = "C", discrete = F,  direction = -1,
                     breaks = brks, labels = lbls,
                     name = "ASFR ratio") +
  geom_contour(bins = 20, col = "black", size = .15, alpha = 0.8) +
  redo_lexis_shape(pmin,pmax,amin,amax, interval = 5)

# with log

df_ratio %>%
  ggplot() +
  aes(x = year, y = age, z = log_ratio) +
  geom_tile(aes(fill = log_ratio)) +
  # adding axis titles
  scale_fill_viridis(option = "C", discrete = F,  direction = -1,
                     # breaks = brks, labels = lbls,
                     name = "ASFR ratio") +
...
# adding right labels for that...

brks <- quantile(c(min(df_ratio$log_ratio), max(df_ratio$log_ratio)), probs = seq(0, 1, 0.25))
