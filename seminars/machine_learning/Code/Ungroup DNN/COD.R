...
country_cover <- DD %>% select(country,Year) %>%group_by(country) %>%  
  mutate(Year_min=min(Year),Year_max=max(Year)) %>% select(-Year) %>% unique()
...
DD <- DD %>% filter(Year>1969&Year<2015)
train = DD[Year < 2001]
test = DD[Year >= 2001]
...
Age_embed = Age %>% 
  layer_embedding(input_dim = 111, output_dim = 10,input_length = 1, name = 'Age_embed') %>%
  keras::layer_flatten()
...
Country_embed = Country %>% 
  layer_embedding(input_dim = 50, output_dim = 10,input_length = 1, name = 'Country_embed') %>%
  keras::layer_flatten()


main_output <- layer_concatenate(list(e0,Year,Age_embed,Sex_embed,Country_embed
)) %>% 
  layer_dense(units = 300, activation = 'sigmoid') %>% #era 300 unit
 # layer_dropout(0.10) %>% 
   layer_dense(units = 300, activation = 'sigmoid') %>% #era 300 unit
  # layer_dropout(0.10) %>% 
...
model = load_model_hdf5("multipop_param.h5") #parametri del paper
...
test %>% filter(country=="ITA",Sex=="M",Year==2014) %>% ggplot(aes(Age,log(mx_5)))+geom_line()+
  geom_line(aes(Age,log(mx_hat),col="red"))


test %>% filter(Year==2014,Sex=="M") %>% ggplot(aes(Age,log(mx_5)))+geom_point(size=0.4)+
  geom_line(aes(Age,log(mx_hat)),col="red")+ facet_wrap(country~.)+ggtitle("Male 2014")


test %>% filter(Year==2014,Sex=="F") %>% ggplot(aes(Age,log(mx_5)))+geom_point(size=0.4)+
  geom_line(aes(Age,log(mx_hat)),col="red")+ facet_wrap(country~.)+ggtitle("Female 2014")


test %>% filter(Year==2001,Sex=="M") %>% ggplot(aes(Age,log(mx_5)))+geom_point(size=0.4)+
  geom_line(aes(Age,log(mx_hat)),col="red")+ facet_wrap(country~.)+ggtitle("Male 2001")


test %>% filter(Year==2001,Sex=="F") %>% ggplot(aes(Age,log(mx_5)))+geom_point(size=0.4)+
  geom_line(aes(Age,log(mx_hat)),col="red")+ facet_wrap(country~.)+ggtitle("Female 2001")
...
floor(mean(final_table$MAE_M)* 100) / 100
floor(mean(final_table$RMSE_M)* 100) / 100
floor(mean(final_table$MAE_F)* 100) / 100
floor(mean(final_table$RMSE_F)* 100) / 100
